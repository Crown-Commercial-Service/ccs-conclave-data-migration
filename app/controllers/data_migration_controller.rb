    ...
    def process_json_data(json_data)
        ...
        if validator.validate(json_data)
            ...
            json_data.each do |org|
                ...
                @ppg_migration_service = Migrate::Ppg.new(cii_migration_service_response[:response_status_code], cii_migration_service_response[:response_body])
                ppg_migration_service_response_org = @ppg_migration_service.migrate_org(org) # { response_status_code: <code>, response_body: nil }
                #ppg_migration_service_response_users = @ppg_migration_service.migrate_users(org) # { response_status_code: <code>, response_body: nil }

                @data_migration_service = Migrate::DataMigration.new(cii_migration_service_response[:response_status_code], ppg_migration_service_response_org[:response_status_code], 'ppg_migration_service_response_users[:response_status_code]')
                @data_migration_service.migrate_org(org)
